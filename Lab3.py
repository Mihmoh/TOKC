    ...
    def input_symbol(self, symbol):
        ...
        if symbol == '\n' and self.sent_bytes != 0:
            if self.pack != "":
                ...
                print("FCS: ", self.fcs)
                self.parity_bitter(self.pack)
                self.fcs = self.fcs + self.parity_bit
                print("FCS with parity bit: ", self.fcs)
...
def statuser(header, pack, fcs_length, row):
    pack = header + pack
    print("Length of pack: ", len(pack))
    print("Length of fcs: ", fcs_length)
    status_window.textbox.config(state="normal")
    if len(pack) >= 41:
        fcs = pack[-6:]
        pack = pack[:35]
        status_window.textbox.insert(tk.END, pack)
        status_window.textbox.insert("%d.%d" % (row, len(pack) - fcs_length),
                                     fcs, "BLUE")
    else:
        status_window.textbox.insert(tk.END, pack)
    ...
    while True:
        ...
        while len(output_str) > len(pack):
            ...
            if index != 0 and index != -1:
                ...
                data = fcs_deformer(header, data_and_fcs, row)
                print("Data: ", data)
